...
function engine(){
  ...
  if (seasonTime <= 0){
    ...
    achievementUnlocked("<code style=font-size:20px;>"+seasons[currentSeason]+" has started!</code>",false)
	  ...
	  if (currentSeason == 1){
        document.getElementById("demo").innerHTML = "It's summer, which means temperatures have risen and the air is dryer. There is less rain for crops. Despite this the sun is out and plants are growing. Farmers are beginning to save crops for the colder months and futures makes sure they will make money.  ";

     }
     if (currentSeason == 2){
        document.getElementById("demo").innerHTML = "As temperature starts to drop in the fall so does production. Farmers have to dedicate more time and money towards their crops. And as you will see some crops don't grow in certain seasons. With futures farmers can lock in the prices for future crops so that even if they are not producing at the time, in the future they won't lose money. ";

     }
	 
	  if (currentSeason == 3){
        document.getElementById("demo").innerHTML = "Now that its winter its even harder for farmers to produce. This means that prices will fluctuate with higher demand but also higher cost of labor. With futures farmers can assure that they will not lose money with prices changing. They can also make sure that when they start farming again the price will not change  ";

     }
	  if (currentSeason == 0){
        document.getElementById("demo").innerHTML = "The weather is getting warmer again so its ideal weather for farming. After the winter farmers are eager to get back to work. Futures ensure that as they start producing again they will be making money. With more competition now farmers can also ensure that buyers won't back out of a deal or pay less. ";
